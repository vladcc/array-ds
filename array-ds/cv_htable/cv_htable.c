...
static void * clean_up(cv_htbl * cvht, int buckets_made)
{
    ...
    for (int i = 0; i < buckets_made; ++i)
        c_vect_destroy(c_vect_get(the_tbl, i));
    ...
    )
{
    ...
    unsigned int hash = cvht->hash_fun(key, cvht->elem_size);
    int index;
    void * found = c_vect_find_ind(
        c_vect_get(the_tbl, hash%c_vect_length(the_tbl)),
        key, &index
    );
...
static cv_htbl stop_world_n_rehash(cv_htbl * old_cvht, bool * success)
{
    ...
    fcomp compar = c_vect_compar(c_vect_get(old_tbl, 0));
        ...
        ))
    {
        ...
        {
            if ((bucket = c_vect_get(old_tbl, i)) && !c_vect_is_empty(bucket))
            {
...
const void * cv_htbl_insert(cv_htbl * cvht, const void * key)
{
    ...
    if (!ret)
    {
        ...
        c_vector * the_bucket = c_vect_get(the_tbl, nbucket);
...
void * cv_htbl_remove(cv_htbl * cvht, const void * key)
{
    ...
    {
        c_vect_remove_at(
            c_vect_get(the_tbl, out_hash%c_vect_length(the_tbl)), out_index
            );
        --cvht->elem_inside;
    }
